"""
CIFAR-10 MS Loss + SNGP with WRN-28-10 depth/width but NO skip connections.

Ablation of cifar10_ms_sngp_sn.py: keeps the same WRN-28-10 channel structure
[160, 320, 640], depth (28 layers), spectral norm (coeff=6.0), MS Loss, and
augmented data — but removes all residual skip connections.

Why this ablation?
  Skip connections in ResNets mean the Lipschitz constant of a residual block
  is 1 + Lip(F), not just Lip(F). Removing them gives a cleaner product bound:
  Lip(net) <= prod_i coeff_i, where coeff_i = 0.95 per SN layer. This tests
  whether skip connections help or hurt when distance structure in the feature
  space (enforced by MS Loss + SN) is what matters for GP uncertainty.
...
Usage:
    # Fresh training
    python experiments/cifar10_ms_sngp_no_skip.py \\
        --config configs/experiment_april4_ms_sngp_no_skip.yaml --seed 0

    # Auto-resume a crashed job
    python experiments/cifar10_ms_sngp_no_skip.py \\
        --config configs/experiment_april4_ms_sngp_no_skip.yaml --seed 0 \\
        --run-name ms_sngp_no_skip_seed0 --auto-resume
"""
...
from src.models.sngp import WRNNoSkipSupConSNGPClassifier, laplace_predictive_probs
...
def train_epoch(
    ...
    progress = tqdm(loader, desc=f"MS-SNGP-NoSkip Epoch {epoch}", leave=False)
...
def main(cfg: dict) -> None:
    ...
    # ── Model ───────────────────────────────────────────────────────────────────
    model_cfg = cfg["model"]
    model = WRNNoSkipSupConSNGPClassifier(
    ...
    if run is not None:
        ...
        if saved_checkpoints:
            ...
            import wandb
            artifact = wandb.Artifact("cifar10_ms_sngp_no_skip_best_model", type="model")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CIFAR-10 MS Loss + SNGP + no skip connections")

"""
CIFAR-10 Multi-Similarity Loss + SNGP with VGG-style CNN backbone.

Ablation of cifar10_ms_sngp.py: replaces the WRN-28-10 encoder with a
VGG-style CNN (8 conv layers, no skip connections, spectral norm throughout).

Why CNN vs ResNet?
  Skip connections in ResNet complicate Lipschitz analysis — the Lipschitz
  constant of a residual block is 1 + Lip(F), not just Lip(F). A plain
  CNN without skip connections gives a cleaner product bound: Lip(net) ≤
  prod_i σ_i, where each σ_i ≤ coeff = 0.95. This tests whether the
  bi-Lipschitz constraint benefits from a simpler architecture.

Usage:
    python experiments/cifar10_ms_sngp_cnn.py \\
        --config configs/experiment_april4_ms_sngp_cnn.yaml --seed 0
"""
...
from src.models.sngp import CNNSupConSNGPClassifier, laplace_predictive_probs
...
def train_epoch(
    ...
    progress = tqdm(loader, desc=f"MS-SNGP-CNN Epoch {epoch}", leave=False)
...
def main(cfg: dict) -> None:
    ...
    if run is not None:
        ...
        if saved_checkpoints:
            ...
            import wandb
            artifact = wandb.Artifact("cifar10_ms_sngp_cnn_best_model", type="model")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CIFAR-10 MS Loss + SNGP + CNN backbone")

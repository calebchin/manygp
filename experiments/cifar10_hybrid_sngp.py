"""
CIFAR-10 Hybrid (SupCon + MS Loss) + SNGP with SN WRN-28-10 backbone.

Combines supervised contrastive loss and multi-similarity loss jointly with
cross-entropy, all on the same GP-input embeddings from a spectrally-normalized
WRN-28-10 backbone.

Why combine both metric losses?
  SupCon provides a uniform gradient signal across all valid positive pairs,
  giving stable global cluster structure. MS Loss reweights pairs by hardness —
  near-boundary pairs get high gradient, easy pairs near zero. Together they
  complement each other: MS Loss sharpens the boundaries SupCon establishes.
  Both operate on L2-normalized cosine similarity, so they are geometrically
  compatible without rescaling.

Loss = supcon_weight * L_supcon + ms_weight * L_ms + ce_weight * L_ce

  SupCon: gp_inputs reshaped to (B, 2, D) — standard SupCon view format
  MS Loss: gp_inputs flat (B*2, D) — treats both views as independent samples
  CE:     logits (B*2, 10) vs ce_labels (B*2,)
...
Usage:
    # Fresh training
    python experiments/cifar10_hybrid_sngp.py \\
        --config configs/experiment_april4_hybrid_sngp.yaml --seed 0

    # Auto-resume a crashed job (run from cluster, same args as training)
    python experiments/cifar10_hybrid_sngp.py \\
        --config configs/experiment_april4_hybrid_sngp.yaml --seed 1 \\
        --run-name hybrid_sngp_seed1 --auto-resume
"""
...
from src.training.contrastive import MultiSimilarityLoss, SupConLoss
...
def train_epoch(
    ...
    supcon_loss_fn, ms_loss_fn, ce_loss_fn,
    device, epoch,
    supcon_weight, ms_weight, ce_weight,
    run=None, log_every_steps=None, global_step=0,
):
    model.train()
    running_total = running_supcon = running_ms = running_ce = 0.0
    total_correct = total_examples = 0

    progress = tqdm(loader, desc=f"Hybrid-SNGP Epoch {epoch}", leave=False)
    for views, labels in progress:
        ...
        # SupCon expects (B, V, D)
        supcon_loss = supcon_loss_fn(gp_inputs.view(B, V, -1), labels)
        # MS Loss uses flat (B*V, D) with repeated labels
        ms_loss = ms_loss_fn(gp_inputs, ce_labels)
        ce_loss = ce_loss_fn(logits, ce_labels)

        total_loss = supcon_weight * supcon_loss + ms_weight * ms_loss + ce_weight * ce_loss
        total_loss.backward()
        optimizer.step()

        global_step += 1
        running_total   += total_loss.item()
        running_supcon  += supcon_loss.item()
    ...
    return {
        ...
        "supcon_loss": running_supcon / len(loader),
...
def main(cfg: dict) -> None:
    ...
    # Load model + optimizer BEFORE creating the scheduler.
    # CosineAnnealingLR(last_epoch=k) requires 'initial_lr' in optimizer param_groups,
    # which only exists after the optimizer state dict is loaded or initial_lr is injected.
    ...
    # ── Loss functions ──────────────────────────────────────────────────────────
    supcon_loss_fn = SupConLoss(temperature=train_cfg.get("supcon_temperature", 0.07))
    ...
    if resume_ckpt_data is not None:
        # model + optimizer + scheduler already loaded above
        global_step = start_epoch * len(train_loader)
        # Use same timestamped dir so new top-k checkpoints land in the right place
        stored_resolved = (
        ...
        if stored_resolved and checkpoint_path:
            ...
            runtime_cfg.setdefault("output", {})["resolved_checkpoint_path"] = resolved_ckpt
        # Pre-populate top-k list from files already on disk
        if resolved_ckpt:
    ...
    # Precision matrix is rebuilt from training batches; reset regardless of resume
    ...
    for epoch in epoch_progress:
        train_metrics, global_step = train_epoch(
            ...
            supcon_loss_fn=supcon_loss_fn, ms_loss_fn=ms_loss_fn, ce_loss_fn=ce_loss_fn,
            device=device, epoch=epoch,
            supcon_weight=supcon_weight, ms_weight=ms_weight, ce_weight=ce_weight,
    ...
    if run is not None:
        ...
        if saved_checkpoints:
            ...
            import wandb
            artifact = wandb.Artifact("cifar10_hybrid_sngp_best_model", type="model")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CIFAR-10 Hybrid (SupCon+MS) Loss + SNGP")
    ...
    parser.add_argument("--auto-resume",     action="store_true", dest="auto_resume",
                        help="Scan the seed checkpoint dir for the latest epoch checkpoint "
                             "and look up the matching W&B run automatically")

"""
CIFAR-100 OOD detection and corruption robustness evaluation.

Loads a trained CIFAR-100 checkpoint (any model type) and evaluates:
  1. In-distribution accuracy (CIFAR-100 test set)
  2. OOD detection vs SVHN (easy) and CIFAR-10 (hard)
     - Dempster-Shafer uncertainty score
     - Max-probability baseline (1 - p_max)
     - Metrics: AUROC, AUPR
  3. CIFAR-100-C: accuracy, ECE, MCE across 15 corruptions × 5 severities
  4. Inference latency (ms/image) for each evaluation

Usage:
    python experiments/cifar100_ood_eval.py --config configs/cifar100_ood_eval.yaml
"""
...
def main(cfg: dict) -> None:
    ...
    # -------------------------------------------------------------------------
    # 1. In-distribution: CIFAR-100 test set
    # -------------------------------------------------------------------------
    print("\n=== In-distribution: CIFAR-100 test ===")
    _, id_loader, _, _ = get_cifar100_loaders(
        data_root=data_root,
    ...
    ood_logits, ood_probs, _, ood_latency = collect_logits_and_probs(
        model_wrapper, cifar10_ood_loader, device, num_mc_samples=num_mc_samples
    )
    cifar10_metrics = evaluate_ood_split(id_logits, id_probs, ood_logits, ood_probs)
    cifar10_metrics["latency_ms_per_image"] = ood_latency
    results["ood"]["cifar10"] = cifar10_metrics
    print(f"  DS  AUROC: {cifar10_metrics['dempster_shafer']['auroc']:.4f}  "
          f"AUPR: {cifar10_metrics['dempster_shafer']['aupr']:.4f}")
    print(f"  p_max AUROC: {cifar10_metrics['max_prob']['auroc']:.4f}  "
          f"AUPR: {cifar10_metrics['max_prob']['aupr']:.4f}")
    print(f"  Latency: {ood_latency:.3f} ms/image")

    if run is not None:
        run.log({
            "ood/cifar10/ds_auroc": cifar10_metrics["dempster_shafer"]["auroc"],
            "ood/cifar10/ds_aupr": cifar10_metrics["dempster_shafer"]["aupr"],
            "ood/cifar10/mp_auroc": cifar10_metrics["max_prob"]["auroc"],
            "ood/cifar10/mp_aupr": cifar10_metrics["max_prob"]["aupr"],
            "ood/cifar10/latency_ms_per_image": ood_latency,
        })

    # -------------------------------------------------------------------------
    # 4. CIFAR-100-C: corruption robustness
    # -------------------------------------------------------------------------
    print("\n=== CIFAR-100-C: corruption robustness ===")
    cifarc_root = ood_cfg.get("cifarc_root", data_root)
    cifarc_dir = maybe_download_cifarc(cifarc_root, "cifar100")
    ...
    results_path = output_cfg.get("results_path", "./results/cifar100_ood_eval.json")
    csv_path = output_cfg.get("csv_path", "./results/cifar100_ood_cifarc.csv")
    ...
    if run is not None:
        ...
        artifact = wandb.Artifact("cifar100_ood_eval_results", type="evaluation")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CIFAR-100 OOD evaluation")

"""
CIFAR-10 DINOv2 frozen-backbone SNGP training.

Uses a frozen DINOv2 ViT-S/14 encoder (loaded via torch.hub) as feature
extractor, then trains a shallow spectrally-normalized MLP + RFGP head on
top. CIFAR-10 images are resized to 224×224 and normalized with ImageNet
statistics for DINOv2 compatibility. OOD images (SVHN, CIFAR-C) are passed
through unchanged — DINOv2Encoder.forward() handles the 32→224 upsample.

Usage:
    python experiments/cifar10_dinov2_sngp.py \
        --config configs/experiment_april20_cifar10_dinov2_sngp.yaml
"""
...
def train_epoch(
    ...
    progress = tqdm(loader, desc=f"DINOv2 SNGP Epoch {epoch}", leave=False, disable=not show_progress)
...
def main(cfg: dict) -> None:
    ...
    with torch.no_grad():
        ...
        _logits, _variances = model(dummy, return_cov=True)
        assert _logits.shape == (2, model_cfg["num_classes"]), f"Unexpected logit shape: {_logits.shape}"
        assert _variances.shape == (2, model_cfg["num_classes"]), f"Unexpected variance shape: {_variances.shape}"
        print("Forward pass shape check passed.")
    del dummy, _logits, _variances
    ...
    if run is not None:
        ...
        if saved_checkpoints:
            ...
            import wandb
            artifact = wandb.Artifact("cifar10_dinov2_sngp_best_model", type="model")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CIFAR-10 DINOv2 frozen-backbone SNGP experiment")

...
# Usage:
#   sbatch submit_april4_ms_sngp_no_skip.sh <SEED> [RUN_NAME]

REPO=/w/20252/davida/manygp/manygp
SEED=${1:?"SEED argument required"}
RUN_NAME=${2:-"ms_sngp_no_skip_seed${SEED}"}
...
echo "Running MS-SNGP NoSkip | seed=${SEED} | run=${RUN_NAME}"

source "$REPO/slurm_scripts/retry_lib.sh"

run_with_retry 3 sta414manygp april_4_experiments "$RUN_NAME" \
    python -u experiments/cifar10_ms_sngp_no_skip.py \

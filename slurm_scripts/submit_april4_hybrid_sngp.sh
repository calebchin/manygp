...
# Usage:
#   sbatch submit_april4_hybrid_sngp.sh <SEED> [RUN_NAME]

REPO=/w/20252/davida/manygp/manygp
SEED=${1:?"SEED argument required"}
RUN_NAME=${2:-"hybrid_sngp_seed${SEED}"}
...
echo "Running Hybrid SNGP (SupCon+MS) | seed=${SEED} | run=${RUN_NAME}"

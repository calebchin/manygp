#!/bin/bash
#SBATCH --job-name=apr2_supcon_sngp
...
# Usage:
#   sbatch submit_april2_supcon_sngp.sh <SEED> <RUN_NAME>
# Example:
#   sbatch submit_april2_supcon_sngp.sh 0 supcon_sngp_seed0

REPO=/w/20252/davida/manygp/manygp
SEED=${1:?"SEED argument required"}
RUN_NAME=${2:-"supcon_sngp_seed${SEED}"}
...
echo "Running experiment_april_2 SupCon+SNGP | seed=${SEED} | run=${RUN_NAME}"
